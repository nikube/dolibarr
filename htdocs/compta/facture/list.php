...
// 	$search_status = '';
// }
$search_paymentmode = GETPOST('search_paymentmode', 'array:intcomma');	// Array from the multiselect combo. A scalar value (old links, saved filters) is converted into an array by GETPOST.
$search_paymentterms = GETPOST('search_paymentterms', 'array:intcomma');	// Array from the multiselect combo. A scalar value (old links, saved filters) is converted into an array by GETPOST.
...
if (is_array($search_paymentmode) && count($search_paymentmode) > 0) {
	foreach ($search_paymentmode as $val) {
		$param .= '&search_paymentmode[]='.urlencode((string) $val);
	}
}
if (is_array($search_paymentterms) && count($search_paymentterms) > 0) {
...
if ($massaction == 'makepayment') {
	...
	$paiementidpreselected = (is_array($search_paymentmode) && count($search_paymentmode) == 1 && (int) reset($search_paymentmode) > 0) ? (int) reset($search_paymentmode) : 0;
	$formconfirm = '';
	$formquestion = array(
